...
namespace GameServer.KiemThe.GameEvents.SpecialEvent
{
    ...
    public static class SpecialEvent_Logic
    {
        ...
        {
            KPlayer client = GameManager.ClientMgr.FindClient(player.RoleID);
			...
			dialog.Selections[-1000003] = "Nhận Túi Tân Thủ";
            ...
            /// Sự kiện Select
            dialog.OnSelect = (x) => {
                SpecialEvent_Logic.OnNPCSelect(npc, player, client, x.SelectID);
            };
            /// Hiện NPCDialog
        ...
        private static void OnNPCSelect(NPC npc, KPlayer player , KPlayer client, int selectionID)
        {
            /// Nếu là đóng khung
            if (selectionID == -999)
            {
            ...
            else if (selectionID == -1000003)
            {
                /// Nhận Hỗ trợ cấp độ tân Thủ
				ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, player, 781, 1, 0, "SpecialEvent_Logic", true, 1, false, Global.ConstGoodsEndTime, "", -1); ///Túi Tân Thủ
				PlayerManager.ShowNotification(client, "Chúc mừng các hạ nhận được Túi Tân Thủ của Kiếm Thánh Mobile Chúc cac hạ bôn tẩu giang hồ vui vẻ");
                /// Đóng Dialog
                 KT_TCPHandler.CloseDialog(player);
            }
            else if (selectionID > -1000)
            {
                ...
                {
                    SpecialEvent_Logic.ShowNotify(npc, client, player, "Thật thứ lỗi, sự kiện không tồn tại!");
                    return;
                }
            ...
            else
            {
                ...
                {
                    SpecialEvent_Logic.ShowNotify(npc, client, player, "Sự kiện chưa mở hoặc đã hết thời gian. Vui lòng quay lại sau!");
                    return;
                }
                ...
                {
                    SpecialEvent_Logic.ShowNotify(npc, client, player, "Có lỗi phát sinh!");
                    return;
                }
                ...
                if (awardInfo.RequireMoney > 0)
                {
                    ...
                    {
                        SpecialEvent_Logic.ShowNotify(npc, client, player, "Số bạc mang theo không đủ!");
                        return;
                    }
                }
                ...
                if (awardInfo.RequireBoundMoney > 0)
                {
                    ...
                    {
                        SpecialEvent_Logic.ShowNotify(npc, client, player, "Số bạc khóa mang theo không đủ!");
                        return;
                    }
                }
                /// Nếu có yêu cầu đồng
                if (awardInfo.RequireToken > 0)
                {
                    ...
                    {
                        SpecialEvent_Logic.ShowNotify(npc, client, player, "Số đồng mang theo không đủ!");
                        return;
                    }
                }
                ...
                if (awardInfo.RequireBoundToken > 0)
                {
                    ...
                    {
                        SpecialEvent_Logic.ShowNotify(npc, client, player, "Số đồng khóa mang theo không đủ!");
                        return;
                    }
                }
                ...
                foreach (SpecialEvent.EventInfo.AwardInfo.RequireItemInfo requireItemInfo in awardInfo.RequireItems)
                {
                    ...
                    {
                        SpecialEvent_Logic.ShowNotify(npc, client, player, "Số lượng nguyên liệu thu thập không đủ!");
                        return;
                    }
                }
                ...
                {
                    SpecialEvent_Logic.ShowNotify(npc, client, player, string.Format("Túi đã đầy, cần sắp xếp tối thiểu {0} ô trống để nhận thưởng!", awardInfo.RequireFreeBagSpace));
                    return;
                }
                ...
                for (int total = 1; total <= awardInfo.AwardCount; total++)
                {
                    ...
                    if (award == null)
                    {
                        SpecialEvent_Logic.ShowNotify(npc, client, player, "Đổi thưởng bị lỗi!");
                        return;
                    }
                ...
                SpecialEvent_Logic.ShowNotify(npc, client, player, "Đổi phần thưởng thành công!");
            }
        }
        ...
        private static void ShowNotify(NPC npc, KPlayer client, KPlayer player, string text)
        {
            KNPCDialog dialog = new KNPCDialog();
            dialog.Text = text;
            dialog.Selections[-999] = "Kết thúc đối thoại";
