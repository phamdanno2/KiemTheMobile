...
namespace GameServer.KiemThe.Logic.Manager.Battle
{
    ...
    public class Battle_SongJin : IEventListener
    {
        ...
        public void MovePlayerToArena(KPlayer KPlayer, int Camp)
        {
            ...
            RegionBD _Find = _BattleConfig.RegionBD.Where(x => x.RegionCamp == Camp).FirstOrDefault();
        ...
        public void intBattle(int Level)
        {
            ...
            {
                Battle_Config = "Config/KT_Battle/Battle_SongJin_HighZone1.xml";
            }
            else if (Level == 5)
            {
                Battle_Config = "Config/KT_Battle/Battle_SongJin_HighZone2.xml";
            }
        ...
        public void UpdatePreadingTbTg(bool IsNotiyText)
        {
            long SECC = (LastTick + _BattleConfig.RegisterDualtion * 1000) - TimeUtil.NOW();

            int FinalSecc = (int)(SECC / 1000);

            string MSG = "Chiến trường sắp bắt đầu, thời gian còn: [" + FinalSecc + "] giây.";
			KTGlobal.SendSystemEventNotification(MSG);
        }
        ...
        private void DoActionSelect(BattlePlayer findPlayer, NPC npc, TaskCallBack x)
        {
            ...
            else if (x.SelectID == -3)
            {
                ...
                else
                {
                    ...
                    if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, findPlayer.Player, ITEMLEVEL_MAU, 1, 0, "TONGKIMRUONG", false, 1, false, Global.ConstGoodsEndTime, "", -1, "", 0, 1, false))
                    {
                        PlayerManager.ShowNotification(findPlayer.Player, "Có lỗi khi nhận rương");
                    }
        ...
        public void ReviceAward(BattlePlayer _Player, NPC npc, int X)
        {
            if (!_Player.IsReviceAward)
            {
                ...
                if (_Find != null)
                {
                    if (!KTGlobal.IsHaveSpace(7, _Player.Player))
                    {
                        PlayerManager.ShowNotification(_Player.Player, "Cần ít nhất 7 ô trống để nhận thưởng");
                        return;
                    }
        ...
        private void ProsecBattle(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (BATTLESTATE == BattelStatus.STATUS_PREPARE)
                {
                    ...
                    // Cứ 1p update thông tin 1 lần
                    if (TimeUtil.NOW() - LastNofity >= 25 * 1000 && BATTLESTATE == BattelStatus.STATUS_PREPARE)
                    {
                        // LastNofity = TimeUtil.NOW();
                        UpdatePreadingTbTg(true);
                    }
                ...
                else if (BATTLESTATE == BattelStatus.STATUS_START || BATTLESTATE == BattelStatus.STATUS_PREPAREEND)
                {
                    ...
                    {
                        KTGlobal.SendSystemEventNotification("Chỉ còn 5 phút nữa chiến trường Tống Kim [" + KTGlobal.CreateStringByColor(_BattleConfig.BattleName, ColorType.Done) + "] sẽ kết thúc,Các anh hũng hào kiệt hãy mau đẩy lui quân địch!");
