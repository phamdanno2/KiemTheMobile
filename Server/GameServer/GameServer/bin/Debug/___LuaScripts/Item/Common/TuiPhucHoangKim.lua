...
function TuiPhucHoangKim:OnUse(scene, item, player, otherParams)
	...
	-- if usedTimeToday >= USED_MAXLIMIT
	
		-- player:AddNotification("Bạn đã dùng " .. USED_MAXLIMIT .. " túi quá số lượng túi phúc hoàng kim trong ngày!")
		-- return
	-- end	
	...
	if usedTimeToday < USED_LIMIT then
		...
		szThongBaoThem = szThongBaoThem.."";
    else 
		-- Thêm bạc khóa
		Player.RemoveItem(player, item:GetID())	-- Xóa vật ph
		Player.AddMoney(player, 100, 0)
		szThongBaoThem = szThongBaoThem.."\nSử dụng quá số lượng trong ngày chỉ nhận được 100 bạc khóa !!!";
	end
	
	-- Thông báo
	player:AddNotification("Sử dụng thành công. Hôm nay đã dùng " .. usedTimeToday .. " " .. item:GetName() .. "" .. szThongBaoThem .. ".")
	-- ************************** --
