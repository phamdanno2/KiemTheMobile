...
namespace FSPlay.GameFramework.Controls
{
    /// <summary>
    ...
    public class PreGameTCPCmdHandler : TTMonoBehaviour
    {
        ...
        /// <summary>
        /// Sự kiện lấy thông tin tân thủ thôn
        /// </summary>
        public Action<List<int>> GetNewbieVillage { get; set; }
        ...
        /// </summary>
        public void CloseSocket(bool isError)
        {
            ...
            this.SocketClosed?.Invoke(isError);
            /// Xóa dữ liệu Handler
        ...
        private void GameSocketFailed(object sender, SocketConnectEventArgs e)
        {
            ...
            MainGame.Instance.QueueOnMainThread(() =>
            {
                if (KuaFuLoginManager.DirectLogin())
                {
                    ...
                    {
                        if (StdErrorCode.Error_Redirect_Orignal_Server == errorCode)
                        {
                            e.ShowMsgBox = false;
                        ...
                        else if (StdErrorCode.Error_Connection_Closing2 == errorCode)
                        {
                            e.ShowMsgBox = false;
                ...
                if (e.ShowMsgBox)
                {
                    if (errorCode == StdErrorCode.Error_Connection_Closing2)
                    {
                        Super.ShowMessageBox("Lỗi đăng nhập", "Tài khoản đã đăng nhập ở thiết bị khác.", true);
                    }
                    else
                    {
                        //Super.ShowMessageBox("Lỗi đăng nhập", "Hệ thống phản hồi quá lâu, tự ngắt kết nối.", true);
                    }
                }
                this.NewTCPGame();
            ...
            MainGame.Instance.QueueOnMainThread(() =>
            {
                if (e.CmdID >= (int) (TCPGameServerCmds.CMD_ROLE_LIST) && e.CmdID <= (int) (TCPGameServerCmds.CMD_CREATE_ROLE))
                {
                    ...
                    else if (e.fields.Length > 1 && Convert.ToInt32(e.fields[0]) < 0)
                    {
                        ...
                        {
                            if (retCode == NameErrorCodes.ErrorServerDisabled)
                            {
                                op = "Máy chủ hiện tại đã đầy, không thể thao tác được. Hãy chọn một máy chủ khác.";
                            }
                            else if (retCode == NameErrorCodes.ErrorInvalidCharacter)
                            {
                                op = "Tên nhân vật có chứa ký tự đặc biệt, hãy chọn một tên khác.";
                            }
                            else if (retCode == NameErrorCodes.ErrorNameHasBeUsed)
                            {
                            ...
                            {
                                op = "Tên nhân vật đã tồn tại, vui lòng chọn một tên khác.";
                            }
                        }
                        Super.ShowMessageBox("Lỗi thao tác", op, () => {
                            this.InitGameError?.Invoke();
                        });
                        return;
                    }
                ...
                else if (e.CmdID == (int) (TCPGameServerCmds.CMD_GETRANDOMNAME))
                {
                    Super.HideNetWaiting();
                }
                else if (e.CmdID == (int) TCPGameServerCmds.CMD_KT_GET_NEWBIE_VILLAGES)
                {
                    if (e.fields != null)
                    {
                        ...
                        try
                        {
                            foreach (string villageIDStr in e.fields)
                            {
                                villages.Add(int.Parse(villageIDStr));
                            }
                            this.GetNewbieVillage?.Invoke(villages);
                        }
                        catch (Exception)
                        {
                            Super.ShowMessageBox("Lỗi", "Không thể tải dữ liệu danh sách tân thủ thôn. Hãy thử lại lần nữa.", true);
                        }
                    }
                }
