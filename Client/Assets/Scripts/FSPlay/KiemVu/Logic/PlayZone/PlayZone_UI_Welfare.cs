...
public partial class PlayZone
{
    ...
    public void OpenUIWelfare(bool isDefaultShowFirstRecharge = false)
    {
        ...
        this.UIWelfare.UISevenDayLogin.Get = (awardInfo) => {
            /// Gửi yêu cầu nhận quà
            GameInstance.Game.SendSpriteGetSevenDayLoginAward(Global.Data.RoleData.RoleID);
        };
        this.UIWelfare.UISeriesLogin.QueryGetSeriesLoginInfo = () => {
            /// Hiển thị màn hình đợi tải
            KTGlobal.ShowLoadingFrame("Đang tải dữ liệu phúc lợi đăng nhập tích lũy...");
        ...
        this.UIWelfare.UISeriesLogin.Get = (awardInfo) => {
            /// Gửi yêu cầu nhận quà
            GameInstance.Game.SendSpriteGetSeriesLoginAward(Global.Data.RoleData.RoleID, awardInfo.ID);
        };
