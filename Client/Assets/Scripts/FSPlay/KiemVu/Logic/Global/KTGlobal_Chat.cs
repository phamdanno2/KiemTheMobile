...
namespace FSPlay.KiemVu
{
    ...
    public static partial class KTGlobal
    {
        ...
        {
            if (PlayZone.GlobalPlayZone.UIChatBoxMini != null)
            {
                PlayZone.GlobalPlayZone.UIChatBoxMini.AddMessage(new SpriteChat()
                {
        ...
        public static string DisplayChat(SpriteChat spriteChat)
        {
            ...
            switch (spriteChat.Channel)
            {
                ...
                {
                    text = string.Format("<color=#b67cfe>[Gia tộc]</color> <link=\"SelectRole\"><color=#5dabfd>{0}</color></link>: {1}", spriteChat.FromRoleName, spriteChat.Content);
                    break;
                }
        ...
        public static void OpenPrivateChatBoxWith(string playerName)
        {
            ...
            /// Mở khung Chat
            PlayZone.GlobalPlayZone.ShowUIChat();
        }
