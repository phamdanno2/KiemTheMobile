...
namespace FSPlay.KiemVu.Logic
{
    ...
    public static class KTLeaderMovingManager
    {
        ...
        /// </summary>
        public static UnityEngine.Vector2 LeaderMoveToPos { get; set; }
        ...
        /// </summary>
        public static UnityEngine.Vector2 LeaderPositiveMoveToPos { get; set; }
        ...
        public static void AutoFindRoad(Point toPos, Action moveDone)
        {
            ...
            PlayZone.GlobalPlayZone.HideProgressBar();

            /// Nếu bị kẹt
            if (!Global.Data.GameScene.CanMoveByWorldPos(leader.Coordinate))
            {
                /// Lấy vị trí thỏa mãn không có vật cản xung quanh
                UnityEngine.Vector2 gridPos = KTGlobal.WorldPositionToGridPosition(leader.PositionInVector2);
                Point noObsPos = KTGlobal.GetRandomNoObsPointAroundPos(new Point((int) gridPos.x, (int) gridPos.y));
                /// Chuyển sang tọa độ thực
                UnityEngine.Vector2 worldPos = KTGlobal.GridPositionToWorldPosition(new UnityEngine.Vector2(noObsPos.X, noObsPos.Y));
                /// Tìm vị trí gần nhất không có vật cản xung quanh
                leader.Coordinate = new Point((int) worldPos.x, (int) worldPos.y);
            }

            /// Vị trí đầu cuối
            UnityEngine.Vector2 fromVector = leader.PositionInVector2;
            UnityEngine.Vector2 toVector = new UnityEngine.Vector2(toPos.X, toPos.Y);
            ...
            /// Thực hiện tìm đường
            List<UnityEngine.Vector2> paths = Global.Data.GameScene.FindPath(leader, ref fromVector, ref toVector);
            ...
            Queue<UnityEngine.Vector2> queue = new Queue<UnityEngine.Vector2>();
            ...
            while (queue.Count > 0)
            {
                UnityEngine.Vector2 node = queue.Dequeue();
            ...
            KTLeaderMovingManager.LeaderMoveToPos = new UnityEngine.Vector2(toPos.X, toPos.Y);
        ...
        /// </summary>
        /// <param name="joyPos"></param>
        /// <param name="moveTime"></param>
        public static void MoveByDirVector(UnityEngine.Vector2 dirVector, float distance)
        {
            /// Nếu JoyStick Pos về 0 thì đã dừng thao tác
            if (dirVector == UnityEngine.Vector2.zero)
            {
                return;
            }
            ...
            /// Vị trí bắt đầu
            UnityEngine.Vector2 fromPos = leader.PositionInVector2;
            /// Vị trí đích đến
            UnityEngine.Vector2 toPos = KTMath.FindPointInVectorWithDistance(fromPos, dirVector, distance);

            /// Nếu không có đường đi
			if (!KTGlobal.HasPath(fromPos, toPos))
            {
                /// Tìm vị trí mới trên đường đi mà không có điểm Block
                toPos = KTGlobal.FindLinearNoObsPoint(fromPos, toPos);

                /// Nếu vẫn không có đường đi
                if (!KTGlobal.HasPath(fromPos, toPos))
                {
                    KTGlobal.AddNotification("Vị trí đích không thể đến được!");
                    return;
            ...
            /// Thực hiện tìm đường
            List<UnityEngine.Vector2> paths = Global.Data.GameScene.FindPath(leader, ref fromPos, ref toPos);
