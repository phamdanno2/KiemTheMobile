...
namespace FSPlay.KiemVu
{
    ...
    public static class Utils
    {
        ...
        public static bool IsClickOnGUI(RectTransform transform)
        {
            if (Application.isEditor || Application.platform == RuntimePlatform.WindowsPlayer)
            {
                if (Input.GetMouseButtonDown(0) && RectTransformUtility.RectangleContainsScreenPoint(transform, Input.mousePosition, Global.MainCamera))
                {
                    return true;
                }
            }
            else
            {
                if (Input.touchCount > 0)
                {
                    ...
                    if (touch.phase == TouchPhase.Began && RectTransformUtility.RectangleContainsScreenPoint(transform, touch.position, Global.MainCamera))
                    {
                        return true;
                    }
                }
