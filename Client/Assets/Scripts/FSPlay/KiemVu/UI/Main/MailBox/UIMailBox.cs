...
using UnityEngine;
using TMPro;
using FSPlay.KiemVu.UI.Main.Mail;
...
namespace FSPlay.KiemVu.UI.Main
{
    ...
    public class UIMailBox : MonoBehaviour
    {
        ...
        /// <summary>
        /// Text đồng khóa đính kèm
        /// </summary>
        [SerializeField]
        private TextMeshProUGUI UIText_BoundToken;
        ...
        private void RefreshData()
        {
            ...
            if (this.currentMailData.HasFetchAttachment == 1)
            {
                ...
                this.UIText_BoundMoney.text = KTGlobal.GetDisplayMoney(this.currentMailData.BoundMoney);
                /// Nếu có bạc khóa
                if (this.currentMailData.BoundMoney > 0)
                {
                    hasSomethingToTake = true;
                }
                /// Cập nhật số đồng khóa
                this.UIText_BoundToken.text = KTGlobal.GetDisplayMoney(this.currentMailData.BoundToken);
                /// Nếu có đồng khóa
                if (this.currentMailData.BoundToken > 0)
                {
                    hasSomethingToTake = true;
                }
            }
