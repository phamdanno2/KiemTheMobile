...
using FSPlay.KiemVu.UI.Main.SeashellCircle;
        ...
        private void UpdateStage()
        {
            ...
            else
            {
                ...
                switch (pair.Key.Type)
                {
                    ...
                    {
                        this.UIText_AwardDesc.text = string.Format("Nhận được <color=green>{0}</color> <color=yellow>Đồng khóa</color> hoặc đổi được <color=green>{1}</color> <color=yellow>[{2}]</color>", KTGlobal.GetDisplayMoney(value * multiply), totalSeashells, KTGlobal.GetItemName(KTGlobal.SeashellItemID));
                        break;
                    }
                    default:
                    {
