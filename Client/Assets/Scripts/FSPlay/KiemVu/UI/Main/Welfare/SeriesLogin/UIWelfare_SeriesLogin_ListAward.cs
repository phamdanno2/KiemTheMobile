...
namespace FSPlay.KiemVu.UI.Main.Welfare.SeriesLogin
{
    /// <summary>
    /// Ô phần quà móc tương ứng
    /// </summary>
    public class UIWelfare_SeriesLogin_ListAward : MonoBehaviour
    {
        #region Define
        /// <summary>
        /// Text số ngày
        /// </summary>
        [SerializeField]
        private TextMeshProUGUI UIText_Days;
        ...
        public int Day
        {
            ...
            {
                this._Day = value;
                this.UIText_Days.text = value.ToString();
            }
        }
        #endregion
        ...
        private void UpdateStateToAllItems()
        {
            foreach (Transform child in this.transformItemsList.transform)
            {
                if (child.gameObject != this.UI_ItemPrefab.gameObject)
                {
                    UIWelfare_SeriesLogin_ListAward_Item uiItemBox = child.GetComponent<UIWelfare_SeriesLogin_ListAward_Item>();
